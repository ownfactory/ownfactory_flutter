...
import 'package:migration/versions/version_1.dart';
import 'package:migration/versions/version_empty.dart';
...
void main(List<String> arguments) {
  ...
  migration.runMigrations([
    MigrationInfo(version: 1, builder: (migration) => MigrationVersionEmpty(migration)),
    MigrationInfo(version: 2, builder: (migration) => MigrationVersionEmpty(migration)),
    MigrationInfo(version: 3, builder: (migration) => MigrationVersion1(migration)),
    MigrationInfo(version: 4, builder: (migration) => MigrationVersionEmpty(migration)),
    MigrationInfo(version: 5, builder: (migration) => MigrationVersionEmpty(migration)),
  ]).catchError((e, st) {
    logError('${e.toString()}\n${st.toString()}');
  });
}
